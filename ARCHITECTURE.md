...
| `rscfuse`          | FUSE overlay: mounts `/rsc/<target>/` backed by rsbeacon     |
...
- Notify fd passed to parent `rsc` via `SCM_RIGHTS`, then inherited by
  `rsclient` via `execve`
...
rsc --ctl seccomp -- /bin/ls /tmp
  ...
  │    │
  │    └─ parent: recv notify_fd via SCM_RIGHTS
...
The seccomp filter is intentionally minimal — only syscalls that have no
filesystem path component and cannot be routed through a FUSE mount.
